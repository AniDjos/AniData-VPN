#!/bin/bash
# Script de démarrage pour AniData VPN - 165 pays
# Version utilisant Tkinter (pas de PyQt5/PyQtWebEngine requis)
...
check_dependency() {
    if ! python3 -c "import $1" &>/dev/null; then
        ...
        if ! sudo apt-get install -y python3-$1 2>/dev/null; then
            echo "Tentative d'installation avec pip..."
            pip3 install --user $1 || {
                echo "Impossible d'installer $1. L'application fonctionnera avec des fonctionnalités limitées."
            }
        fi
    fi
}
...
# Créer un fichier Python simplifié qui utilise uniquement pack (pas de grid)
cat > vpn_tk.py << 'EOF'
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
...
# Lancer l'application
echo -e "${GREEN}Lancement d'AniData VPN avec 165 pays...${NC}"
python3 vpn_tk.py