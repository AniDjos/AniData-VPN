#!/bin/bash
# Script de réparation rapide pour AniData VPN
# Ce script corrige les erreurs d'indentation et lance l'interface avec 165 pays
...
echo -e "${GREEN}[1/4]${NC} Vérification de l'environnement..."
...
check_dependency() {
    if ! python3 -c "import $1" &>/dev/null; then
        ...
        if ! sudo apt-get install -y python3-$1; then
            echo -e "   ${YELLOW}Tentative avec pip...${NC}"
            if ! pip3 install --user $1; then
                echo -e "   ${RED}Échec d'installation de $1${NC}"
                return 1
            fi
        fi
        echo -e "   ${GREEN}$1 installé avec succès${NC}"
    else
        echo -e "   ${GREEN}$1 déjà installé${NC}"
    fi
    return 0
}

# Vérifier et installer les dépendances requises
echo -e "${GREEN}[2/4]${NC} Installation des dépendances..."
check_dependency tk || echo -e "${YELLOW}Installation manuelle requise: sudo apt-get install python3-tk${NC}"
check_dependency numpy
check_dependency matplotlib

# Créer un fichier d'interface tkinter propre
echo -e "${GREEN}[3/4]${NC} Création d'une interface graphique sans PyQt5..."

# Créer le répertoire de données
mkdir -p data

# Créer un fichier minimal tkinter_ui.py propre
cat > tkinter_ui.py << 'EOF'
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
...
echo -e "${GREEN}[4/4]${NC} Configuration terminée ! Lancement de l'application..."
echo ""
